...
int main(int argc, char **argv) {
  ...
  if (argc != 7) {
    fprintf(stderr, "Usage: %s --ip <ip> --port <port> --thread-number <thread-number>\n", argv[0]);
    exit(1);
  }

  printf("The AlexLIO's web server has been started.....\n");

  listenfd = Open_listenfd(argv[2], argv[4]);
  /*
  信号处理函数
  用来处理僵尸进程
  */
...
void doit(int fd) {
  ...
  //只能处理GET和POST请求，如果不是GET和POST请求的话返回错误
  if (strcasecmp(method, "GET") && strcasecmp(method, "POST")) {
		clienterror(fd, method, "501", "Not Implemented",
			"AlexLIO does not implement thid method");
		return;
     }
	...
	if (strcasecmp(method, "GET") == 0) {
		...
		if (stat(filename, &sbuf) < 0) {
			clienterror(fd, filename, "404", "Not found",
				"AlexLIO couldn't find this file");
			return;
		}
		...
		if (is_static) { //服务静态内容
			if (!(S_ISREG(sbuf.st_mode)) || !(S_IRUSR & sbuf.st_mode)) {
				clienterror(fd, filename, "403", "Forbidden",
					"AlexLIO couldn't read the file");
				return;
			}
			...
			if (!(S_ISREG(sbuf.st_mode)) || !(S_IXUSR & sbuf.st_mode)) {
				clienterror(fd, filename, "403", "Forbidden",
					"AlexLIO couldn't run the CGI program");
				return;
			}
...
void serve_static(int fd, char *filename, int filesize) {
  ...
  sprintf(body, "%sServer: AlexLIO's Web Server\r\n", body);
...
void serve_dynamic(int fd, char *filename, char *cgiargs) {
  ...
  sprintf(buf, "Server AlexLIO's Web Server\r\n");
  Rio_writen(fd, buf, strlen(buf));
...
void clienterror(int fd, char *cause, char *errnum, char *shortmsg,
  ...
  sprintf(body, "<html><title>AlexLIO Error</title>");
  sprintf(body,
          "%s<body bgcolor="
          "white"
          ">\r\n",
  ...
  sprintf(body, "%s<center><h3>%s: %s</h3></center>\r\n", body, longmsg, cause);
  sprintf(body, "%s<hr><center>The AlexLIO's Web server</center>\r\n", body);
