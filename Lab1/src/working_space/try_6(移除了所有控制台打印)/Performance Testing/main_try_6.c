...
int main(int argc, char* argv[]){
	...
	//拼接输出文件名
	strcat(outFileName,"PS=");	//POOL_SIZE
	char tmp[20];
	snprintf(tmp, sizeof(tmp), "%d", POOL_SIZE);
	...
	printf("total solved:%d\n",total_solved);
        printf("total Spend:%.5lf s to finish.\n",time_diff(tvStart_write,tvEnd_write)/1E6 + time_diff(tvStart_cal,tvEnd_cal)/1E6);
	//写入本次参数
