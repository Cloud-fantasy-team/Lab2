...
void print(int sudoku[][10]){
	ofstream data2;
    data2.open("test1.txt",ios::app);   //打开文件 
	cout<<"-----------------------"<<endl<<endl;
	for (int i=1;i<=9;i++){
		for (int j=1;j<=9;j++){
			if (sudoku[i][j]){
...
void search(int x,int y){
	...
	if (known[x][y]){//known非0，为数字； 则搜索下一个单元格 
		...
		for (int j=1;j<40;j++){
			...
			if (hang[x][i]==0 && column[y][i]==0 && block[(x-1)/3+1][(y-1)/3+1][i]==0)//判断此数是否合法 
			{
				sudoku[x][y]=i;
				hang[x][i]=1;
				column[y][i]=1;
...
int main(){
	...
	//	cout<<"-4-"<<endl;
		...
		cnt=0;
		f=0;
		memset(hang,0,sizeof(hang));//初始化为0 
	...
	return 0;
}//本代码为数独生成代码，思路为CSDN提供，自己进行了文件生成，具体函数没怎么修改 
